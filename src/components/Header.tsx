...
const LABEL_APP = 'IPQC';
const LABEL_CHECKING = '检测中';
const LABEL_CHECK = '检测';
  ...
  return (
    <header className="app-header flex items-center justify-between px-3 sm:px-5 border-b border-[#2a2a2a]">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <span className="text-sm font-semibold text-white tracking-tight shrink-0">{LABEL_APP}</span>
      ...
      <button onClick={onRun} disabled={loading} className="btn-run flex items-center gap-1.5 shrink-0 ml-2">
        ...
        <span className="text-[13px] sm:text-sm">{loading ? LABEL_CHECKING : LABEL_CHECK}</span>
      </button>
    </header>
  );
}
